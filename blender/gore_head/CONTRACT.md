...
- scale.y = elongation (1.0 = round; slash length comes from this). **Bullets:**
  scale.y is the muzzle distance in metres instead (default 1.0 = distant shot;
  < 0.3 soot, < 0.9 stippling, ~0 contact: stellate tear over bone, seared rim,
  muzzle imprint)
...
(skin, eyes, teeth, gums, tongue, mouth lining), tested with a ring of rays the
width of a bullet, so a shot into the open mouth hits the lips and teeth.
Stray empties (scale.x ~ 0, or more than ~3.5 cm from a layer) are ignored.

What each kind does (research-based, see gore-game/docs/research/01-02 and
REALISM_BIBLE §2.7):
...
- **slash**: an incised cut; the knife line is narrow and the lips gape apart
  ...
  skin's own tissue down to the cut depth (dermis -> fat -> muscle), a blood
  pool in the bed, deep start and a shallow scratch tail. Never opens bone
  (score mark only).
- **blunt**: laceration over bone split along the skin lines (linear or Y),
  ...
  the bone, blood in the bed; swelling and bruise develop with `wound_age`;
  depressed skull fracture when deep; knocked-out teeth near the mouth.
- **burn**: smooth lobed dose field: red band, partial thickness (moist red,
  blister domes that appear after 30 s-5 min and fill over hours, a few peeling
  sheets), full thickness (waxy / leathery, sunk, shrinking and pulling the skin
  toward it), charred core that splits open along real fissures.
...
Blood drips: generated by the skin layer as real geometry: flat rivulets
(2-5 mm wide, 0.1-0.4 mm thick) that leave each wound at the LOWEST point of its
rim, run down the surface with gravity, follow the surface, end in a fuller head
on long runs; the first run is long, others stop early; length grows with
`drip_time` and `bleed`. Bleeding cuts and blunt splits also get a liquid blood
sheet filling their bed. No blood spatter is placed on the victim's own skin
...
point attribute `gh_bone_depth` (distance straight in to the skull / jaw, baked
by `gore.bake_bone_depth()` in `build_gore_system()`); wound walls use it to
reach exactly down to the bone.
...
must look correct with no gore at all.

### Global controls (`GH_Controls` custom properties, all 0..1)

`damage`, `bleed`, `drip_time`, `wetness`, `blood_age`, `bruising`, `swelling`,
`wound_age`, `skin_tone`, `pallor`, as defined in `gh_common.CONTROL_PROPS`.
`wound_age` is the time since the injuries (hours = 48 x age^2: 0 = this instant,
0.2 = 2 h, 1 = 48 h): bruises go from redness to a purple bruise to green-yellow
margins (never before ~18 h), swelling builds up, burn blisters appear and fill.
Materials drive their own node values from `wetness`, `blood_age`, `skin_tone`,
`pallor`, `wound_age`; the GN modifiers drive from `damage`, `bleed`,
`drip_time`, `bruising`, `swelling`, `wound_age`.
