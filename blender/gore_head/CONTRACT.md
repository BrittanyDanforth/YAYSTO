...
skin and muscle also to the mouth lining -- baked by `gore.bake_bone_depth()` in
`build_gore_system()`); wound walls use it to reach exactly down to the bone and
never through the cheek into the mouth. The cut skin edge (surface -> first wall
ring) is marked sharp; every wall ring uses the wall material (GH_Fat), which
paints the thin dermis line at its top.
